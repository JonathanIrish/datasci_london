---
title: "Data Analysis Example from London Workshop 2019"
author: "Copyright (c) 2019 by Duncan Murray, All Rights Reserved; Markdown by Jonathan Irish & Ducan Murray"
date: "September 2019"
output:
  html_document:
    df_print: paged
...
---
[Describe dataset, expected output, runtime, etc.]. 
...
flow_tsne %>%
	...
	geom_point()
```
```{r Remove Outliers and Improve Plotting}
...
flow_clustered %>%
	...
	head
```
```{r Plot Louvain Analysis on t-SNE}
# plot clusters
flow_clustered %>%
	ggplot(aes(x = TSNE.1, y = TSNE.2, colour = louvain))+
	geom_point()
```
```{r Plot k-Means Analysis on t-SNE}
# plot clusters
flow_clustered %>%
	ggplot(aes(x = TSNE.1, y = TSNE.2, colour = kmean))+
	geom_point()
```
```{r Plot Population Abundance by Sample}
...
```
```{r Save as Rdata Files, warning=FALSE}
